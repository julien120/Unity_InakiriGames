...
public class JudgementManager : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        for(int lane =0; lane<InputKeys.Length; lane++)
        {
            ...
            if (Input.GetKeyDown(inputKey))
            {
                ...
                var nearest = GetNearesetNoteControllerBaseInLane(lane);
                // ノーツがレーンに無ければスキップ
                if (!nearest) continue;

                //最近傍らのノーツを処理すべきタイミング(sec)
                var noteSec = nearest.noteProperty.secBegin;
                //処理すべきタイミングと
                //実際にキーが押されたタイミングの差の絶対値
                var differenceSec = Mathf.Abs(noteSec - PlayerControllers.CurrentSec);
    ...
    //指定したレーン内の最近傍らのノーツを取得(存在しなければnull)
    private NoteControllerBase GetNearesetNoteControllerBaseInLane(int lane)
    {
        // 指定したレーン内のノーツ
        var noteControllers = PlayerControllers.ExistingNoteControllers.Where(x => x.noteProperty.lane == lane);
        //　指定したレーン内のノーツが存在する時
        if (noteControllers.Any())
        {
            // beatの差の絶対値が小さい順に並び替え→先頭の要素を取得して返す
            return noteControllers.OrderBy(x => Mathf.Abs(x.noteProperty.beatBegin - PlayerControllers.CurrentBeat)).First();
    ...
    {
        //指定したレーン内のノーツ
        var noteControllers = PlayerControllers.ExistingNoteControllers.Where(x => x.noteProperty.lane == lane && x.isProcessed);
        //指定したレーン内のノーツが存在する時
        if (noteControllers.Any())
        {
