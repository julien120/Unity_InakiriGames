...
//noteControllerBaseに書かれた抽象クラスを継承しているスクリプト
public class LongNoteController : NoteControllerBase
{
    ...
    private void SetTransform()
    {
        ...
        positionBegin.y = isProcessed ?
            ...
            (noteProperty.beatBegin - PlayerControllers.CurrentBeat) *
            PlayerControllers.ScrollSpeed;
        objBegin.transform.localPosition = positionBegin;

        //終点の座標(beatEndによる指定)
        Vector2 positionEnd = new Vector2();
        positionEnd.x = noteProperty.lane - 2;
        positionEnd.y = (noteProperty.beatEnd - PlayerControllers.CurrentBeat) * PlayerControllers.ScrollSpeed;
