    ...
    private static Dictionary<char, int> LanePairs = new Dictionary<char, int>
{
    ...
    //コンストラクタ BMSファイルの読み込み
    public BmsLoader(string filePath)
    {
        //BMSファイルを読み込み、各行を配列に保持する
    ...
    private void LoadMainDataLine(string line)
    {
        ...
        // match.Groups[1~3]に()で囲った部分が代入されている
        // #nnnxx:というフォーマットに一致していればデータ処理する
        if (match.Success)
        {
            //小節番号
            ...
            {
                //小説の開始beat(measurelengthsの先頭からmeasureNumコブンの合計)
                ...
                for(int i=0;i<objCount; i++)
                {
                    ...
                    //現在小説の長さbeat/objCountを加えることで
                    //現在見ているオブジェクトのbeatが出る
                    float beat = measureStartBeat + (i * measureLengths[measureNum] / objCount);
